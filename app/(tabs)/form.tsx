...
const allChildrenHealthData: ChildHealthRecord[] = [
  {
    ...
    consentForms: [
      {
        id: "c1",
        title: "Phiếu đồng ý tiêm cúm mùa",
        status: "pending",
        date: "05/06/2025",
      },
      {
      ...
      {
        id: "c3",
        title: "Phiếu đồng ý tham gia dã ngoại",
        status: "rejected",
        date: "01/06/2025",
      },
    ],
    medicalResults: [
      {
        id: "m1",
        title: "Khám định kỳ 05/2025",
        result: "Tình trạng sức khỏe bình thường, phát triển tốt.",
        date: "10/05/2025",
      },
      {
        id: "m2",
        title: "Kết quả test cúm A",
        result: "Âm tính.",
        date: "06/06/2025",
      },
    ],
    medicationRecords: [
      {
        id: "med1",
        date: "06/06/2025",
        medication: "Thuốc ho (5ml)",
        confirmedBy: "Y tá Thu Phương",
      },
      {
        id: "med2",
        date: "05/06/2025",
        medication: "Thuốc hạ sốt (1 viên)",
        confirmedBy: "Y tá Minh Anh",
      },
    ],
  },
  {
    ...
    medicalResults: [
      {
        ...
        result: "Mắt trái 10/10, mắt phải 9/10.",
        date: "15/05/2025",
      },
    ],
    medicationRecords: [
      {
        id: "med3",
        date: "01/06/2025",
        medication: "Vitamin C (1 viên)",
        dosage: "1 viên/ngày",
        confirmedBy: "Giáo viên chủ nhiệm",
      },
    ],
  },
  {
...
export default function FormScreen() {
  const [selectedChildId, setSelectedChildId] = useState<string>(
    allChildrenHealthData[0]?.id || ""
  );
  const [currentChildData, setCurrentChildData] =
    useState<ChildHealthRecord | null>(null);

  useEffect(() => {
    if (selectedChildId) {
      const foundChild = allChildrenHealthData.find(
  ...
  const getConsentStatusIcon = (status: ConsentForm["status"]) => {
    switch (status) {
      ...
      case "rejected":
        return { name: "cancel", color: "#dc3545" };
      case "pending":
        return { name: "hourglass-empty", color: "#ffc107" };
      default:
        return { name: "help-outline", color: "#6c757d" };
    }
  };
  ...
  return (
    <ScrollView className="flex-1 bg-white pt-4">
      <View className="px-6 py-4 border-b border-gray-100 flex-row items-center">
        <MaterialIcons name="person" size={24} color="#2260FF" />
        <Text className="text-primary font-semibold text-base ml-2">
          Chọn học sinh:
        </Text>
        <View className="flex-1 ml-3 border border-secondary rounded-lg overflow-hidden">
          <Picker
            ...
            {currentChildData.consentForms.length > 0 ? (
              currentChildData.consentForms.map((consent) => {
                ...
                return (
                  <View
                    key={consent.id}
                    className="flex-row items-center bg-gray-50 p-3 rounded-lg mb-2 border border-gray-200"
                  >
                    <MaterialIcons
                    ...
                    <Text
                      className={`text-sm font-semibold ${
                        statusIcon.color === "#28a745"
                          ? "text-green-600"
                          : statusIcon.color === "#dc3545"
                          ? "text-red-600"
                          : "text-yellow-600"
                      }`}
                    >
              ...
              currentChildData.medicalResults.map((result) => (
                ...
                >
                  ...
                  <Text className="text-sm text-gray-700">
                    <Text className="font-medium">Kết quả:</Text>{" "}
                    {result.result}
                  </Text>
          ...
          <View className="pb-20">
            ...
            {currentChildData.medicationRecords.length > 0 ? (
              currentChildData.medicationRecords.map((med) => (
                ...
                >
                  <Text className="text-base font-semibold text-gray-800 mb-1">
                    <MaterialIcons name="healing" size={16} color="#4CAF50" />{" "}
                    {med.medication} {med.dosage ? `(${med.dosage})` : ""}
                  </Text>
                  <Text className="text-sm text-gray-700">
                    <MaterialIcons name="person" size={14} color="#6B7280" />{" "}
                    Xác nhận bởi: {med.confirmedBy}
                  </Text>
                  <Text className="text-xs text-gray-500 mt-1 text-right">
                    Ngày: {med.date}
                  </Text>
